...
克隆本仓库后，将 `skills/` 下的所有目录复制到 `~/.claude/skills/`，所有项目均可使用：

```bash
git clone https://github.com/your-org/claude-dev-studio.git
cp -r claude-dev-studio/skills/* ~/.claude/skills/
```
...
```powershell
git clone https://github.com/your-org/claude-dev-studio.git
Copy-Item -Recurse claude-dev-studio\skills\* $env:USERPROFILE\.claude\skills\
```
...
将 `skills/` 目录下的文件夹复制到目标项目的 `.claude/skills/` 目录下，仅对该项目生效：
